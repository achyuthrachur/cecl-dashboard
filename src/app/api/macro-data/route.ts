...
interface MacroDataResponse {
  data: Record<MacroIndicator, MacroTimeSeries>
  isLiveData: boolean
  source: 'fred' | 'mock'
}
...
export async function GET(): Promise<NextResponse<MacroDataResponse>> {
  ...
  if (!apiKey) {
    ...
    // Return mock data
    const mockData: Record<string, MacroTimeSeries> = {}
    for (const indicator of INDICATORS) {
      mockData[indicator] = generateMockMacroData(indicator, startDate, endDate)
    }
    return NextResponse.json({
      data: mockData as Record<MacroIndicator, MacroTimeSeries>,
      isLiveData: false,
      source: 'mock'
    })
  }
  ...
  try {
    ...
    const results: Record<string, MacroTimeSeries> = {}

    // Fetch in parallel for better performance
    const promises = INDICATORS.map(async (indicator) => {
      try {
        const data = await fetchFREDSeries(indicator, apiKey, startDate, endDate)
        return { indicator, data, success: true }
      } catch (error) {
        console.error(`Error fetching ${indicator}:`, error)
        return {
          indicator,
          data: generateMockMacroData(indicator, startDate, endDate),
          success: false
        }
      }
    })
    ...
    let allSuccess = true
    for (const { indicator, data, success } of responses) {
      results[indicator] = data
      if (!success) allSuccess = false
    }

    return NextResponse.json({
      data: results as Record<MacroIndicator, MacroTimeSeries>,
      isLiveData: allSuccess,
      source: allSuccess ? 'fred' : 'mock'
    })
  } catch (error) {
